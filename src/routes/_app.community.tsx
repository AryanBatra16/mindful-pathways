...
function Community() {
  ...
  return (
    <div className="flex-1 overflow-y-auto p-4 lg:p-8 bg-background scroll-smooth">
          ...
          <div className="lg:col-span-3 space-y-4">
            ...
            <div className="glass rounded-3xl p-5 shadow-card">
              ...
              <div className="flex items-center justify-between mt-3 flex-wrap gap-2">
                ...
                <button onClick={handleSharePost} className="px-5 py-2 rounded-xl gradient-primary text-white font-medium shadow-soft flex items-center gap-2 cursor-pointer hover:shadow-glow transition-all">
            ...
            <div className="space-y-4">
              {filteredPosts.map((p, i) => (
                ...
                >
                  <div className="flex items-center gap-3 mb-3">
                    <div className="h-10 w-10 rounded-full flex items-center justify-center font-semibold text-white shadow-inner" style={{ background: `var(--${p.color})` }}>
                  ...
                  <div className="flex items-center gap-4 mt-4 pt-3 border-t border-border">
                    <button onClick={() => toggleLikePost(p.id)} className={`flex items-center gap-1.5 text-sm transition-colors group cursor-pointer ${p.liked ? "text-pink-500" : "text-muted-foreground hover:text-pink-500"}`}>
                      <Heart className={`h-4 w-4 group-hover:scale-110 transition-transform ${p.liked ? "fill-pink-500" : ""}`} /> {p.reactions}
                    </button>
              ...
              {filteredPosts.length === 0 && (
                <div className="text-center p-10 glass rounded-3xl text-muted-foreground">No posts found in this section yet.</div>
              )}
            </div>
          </div>
